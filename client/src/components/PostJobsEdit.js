...
export default function Postjobs() {
  ...
  const submitHandler = (event) => {
    ...
    if (change !== takeSuggest) {
      setError({
        ...error,
        location: "Please Provide Location From Suggested Location",
      });
    }
  ...
  const form_nameexist = (
    <>
      <div class="company-wrapper">
        <div class="company-type">
          {" "}
          <p class="title">Select an existing company</p>
          <Dropdown
          ...
          />
          {error.company_id ? (
            <p className="error">Please Select A Company Name</p>
          ) : (
            ""
          )}
        </div>
        <h4>OR</h4>
        <Button
          onClick={() => setState({ ...state, discard: true, company_id: "" })}
          primary
        >
    ...
    <>
      <div className="create-company-wrapper">
        ...
        <div className="website">
          ...
          {error.website ? <p className="error">{error.company_name}</p> : ""}
        </div>
        <div className="company-logo-url">
          <p className="title">Company logo url</p>
          <div className="ui input">
            <input
              name="logo_url"
              onChange={myChangeHandler}
              value={state.logo_url}
              placeholder="http://www.google.com/logo.png"
            />
          </div>
          {error.logo_url ? <p className="error">{error.logo_url}</p> : ""}
        </div>
      </div>
      ...
      <div class="short-description">
        ...
        {error.short_description ? (
          <p className="error">{error.short_description}</p>
        ) : (
          ""
        )}
      </div>
      {state.existing_name ? (
        <Button
          onClick={() =>
            setState({ ...state, discard: false, company_name: "" })
          }
        >
          <Icon name="remove circle" />
          Discard and Select an Company name
        </Button>
      ) : (
        ""
      )}
          ...
          <div className="post-job-form">
            <form onSubmit={submitHandler}>
              <div className="company">
                ...
                {state.existing_name && state.discard === false
                  ? form_nameexist
                  : formnotexist}
              </div>
              <div className="job">
                <div className="horizontal-line">
                  <div className="logo-wrapper">
                    ...
                    <span className="job-logo">
                      <img
                        alt="company logo"
                        src="/images/iconfinder_Sed-05_2232591.png"
                      />
                      a
                    </span>
                    <span>Job</span>
                ...
                <div className="layer1">
                  <div className="job-title">
                    ...
                    {error.job_title ? (
                      <p className="error">{error.job_title}</p>
                    ) : (
                      ""
                    )}
                  </div>
                  ...
                  <div className="remote-type">
                    <p className="title">Remote-working</p>
                    ...
                    />
                    {error.remote ? (
                      <p className="error">{error.remote}</p>
                    ) : (
                      ""
                    )}
                  </div>
                  <div class="job-type">
                    ...
                    />
                    {error.job_type ? (
                      <p className="error">{error.job_type}</p>
                    ) : (
                      ""
                    )}
                  </div>
                ...
                <div class="layer2">
                  <div class="salary">
                    ...
                    {error.salary ? (
                      <p className="error">{error.salary}</p>
                    ) : (
                      ""
                    )}
                  </div>
                  <div class="experience">
                    <div class="experience-type">
                      ...
                      />
                      {error.experience ? (
                        <p className="error">{error.experience}</p>
                      ) : (
                        ""
                      )}
                    </div>
                  ...
                  <div class="apply">
                    <p class="title">Apply url or email</p>
                    <div class="ui input">
                      <input
                        name="apply_link"
                        onChange={myChangeHandler}
                        value={state.apply_link}
                        placeholder="Url or email to use in order to apply"
                      />
                    </div>
                    {error.apply_link ? (
                      <p className="error">{error.apply_link}</p>
                    ) : (
                      ""
                    )}
                  </div>
    ...
    return (
      <div class="advertise-wrapper">
        <div className="icon-briefcase">
          <Icon color="black" name="briefcase" />
        </div>
        <h1>Thank you for your insertion</h1>
        <p>Your insertion has been published , want to promote it?</p>
        <p>
          <Link className="ui large primary button" to="/">
            Go back to Home
          </Link>
          <Link className="ui large primary button" to={`/Featured/${_id}`}>
            Promote my insertion
          </Link>
        </p>
      </div>
    );
