...
const Admin = () => {
  ...
  const funcPageData = () => {
    let row = [];
    for (
      let i = (activePage - 1) * dataPerPage;
      i < activePage * dataPerPage;
      i++
    ) {
      ...
      row.push(
        <tr key={i * 25}>
          <td>{data[i]._id}</td>
          <td>{data[i].company.profile.email}</td>
          <td>{data[i].featured.featured_created_at}</td>
          <td>{data[i].featured.featured_expired_at}</td>
          <td>
            <input
              ...
              type="button"
              value="OFF"
              onClick={(e) => funcFeaturedPostOff(e.target.id)}
            />
          </td>
        </tr>
      );
            ...
            <li>
              <Link to={"/admin"}>
                <img
                  ...
                  alt=""
                  src="/images/iconfinder_pie-chart_322488.png"
                />
            ...
            <li>
              <Link to={"/admin"}>
                <img
                  className="dashboard-icon"
                  alt=""
                  src="/images/iconfinder_table_1608863.png"
                />
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Email</th>
                <th>Featured At</th>
                <th>Featured Experider At</th>
                <th>Featured Off</th>
              </tr>
            </thead>
            <tbody>{funcPageData()}</tbody>
          </table>
          <div className="pagination-wrapper">
            ...
            <div>
              ...
              <input
                ...
                type="Number"
                onKeyDown={(e) =>
                  e.key === "Enter" ? setActivePage(Number(e.target.value)) : ""
                }
              />
            </div>
            <ul>
              {activePage >= 2 ? (
                <li
                  className="pagination pag-arrow-left"
              ...
              {funcPage()}
              {activePage <= Math.ceil(dataLength / dataPerPage) - 2 ? (
                <li
                  className="pagination pag-arrow-right"
                  onClick={() => setActivePage(activePage + 1)}
                >
                  ›
                </li>
